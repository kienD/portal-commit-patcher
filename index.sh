...
update_origin_pr_state() {
  ...
  curl -X PATCH -H "$GITHUB_HEADER" -d "{\"state\": \"${state}\"}" "https://api.github.com/repos/${GITHUB_ORIGIN_USER}/${GITHUB_ORIGIN_REPO}/issues/${ORIGIN_PULL_ID}"
}
...
send_pull_request() {
  ...
  local data="{\"title\": \"${file_name}\", \"body\": \"ci:forward\", \"head\": \"${file_name}\", \"base\": \"7.1.x\"}"
...
main() {
  ...
  local comment="PR forwarded to [here](https://github.com/${GITHUB_DESTINATION_USER}/${GITHUB_DESTINATION_REPO}/pull/${pr_number})"
